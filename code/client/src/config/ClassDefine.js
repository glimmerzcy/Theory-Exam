    ...
    static parseFromRow(que, opts) {
        ...
        opts.split("").forEach(
            (opt, k) =>
                ...
                question.setOption(
                    k,
                    new Option(que[opt], que["答案"].indexOf(opt) >= 0)
                )
        )
        return question
    }
...
export class Section {
    constructor(type = "sc", subtitle = "单选") {
        this.config.type = type
        this.config.subtitle = subtitle
        this.quiz =
            this.config.type === "sub" ? [] : [new Question()]
        if (type === "sc") {
            this.config.num = 40
            this.config.point = 1
        } else if (type === "mc") {
    ...
    quesions = () => this.quiz
    add = quesion => this.quiz.push(quesion)
    addQuestion = () =>
        this.quiz.push(
            this.config.type === "sub" ? new SubQuestion() : new Question()
        )
    calculateScore = () => {
        let { quiz, config } = this,
            { point, num } = config
        let score = quiz.reduce(
                (acc, vec) => (vec ? acc + vec.is_exist * point : acc + 0),
                0
            ),
            sup = point * num
...
export class Paper {
    ...
    calculateScore = () =>
        this.body.reduce((acc, vec) => acc + vec.calculateScore(), 0)
    calculateNum = () =>
        this.body.reduce((acc, vec) => acc + vec.config.num * 1, 0)
    canToExam = () =>
        this.body.reduce(
            (acc, sec) => acc && sec.quesions().length >= sec.config.num,
            true
        )
    resetSoftly = i => (this.body[i].quiz = [])
    numSub = 1
    addSub = () => this.body.push(new Section("sub", `主观${++this.numSub}`))
    addSection = section => this.body.push(section)
    renew = () => {
        this.head.id = undefined
        this.body.forEach(sec => sec.quiz.forEach(que => (que.id = undefined)))
    }

    subjectiveSheetNames = () =>
        this.body
            .filter(ele => ele.config.type === "sub")
            .map(ele => ele.config.subtitle)

    parseFromFile = async e => {
        try {
            let paper = new Paper()
            let subjective_sheet_names = this.subjectiveSheetNames()
            ...
            for (let i = 0; i < subjective_sheet_names.length; i++) {
                if (i + sheet_names.length >= paper.body.length) {
                    paper.addSub()
                }
                ...
                for (let que of this.section(i + sheet_names.length).quiz)
                    que.id &&
                        paper
                            .section(i + sheet_names.length)
                            .add(SubQuestion.EQ(que.id))
            }
            let sheets = XLSX.read(new Uint8Array(e.target.result), {
                type: "array"
            }).Sheets
            ...
            for (let i = 0; subjective_sheet_names[i]; i++) {
                let ques = XLSX.utils.sheet_to_json(
                    sheets[subjective_sheet_names[i]]
                )
                if (!ques || !ques[0] || !ques[1]) break

                for (let que of ques) {
                    paper
                        .section(i + sheet_names.length)
                        .add(SubQuestion.parseFromRow(que))
                    await Time.sleep(1)
                }
            }
    ...
    static parseFromDB = async (res, id, papers) => {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (i >= paper.body.length) {
                paper.addSub()
            }
            await paper.resetSoftly(i)
            paper.section(i).config.num = data[i].draw_quantity
            paper.section(i).config.point = data[i].credit_per_question
            if (+data[i].is_subjective) {
                for (let que of ques) {
    ...
    parseToExcel() {
        ...
        subjective_sheet_names.forEach((name, i) =>
            XLSX.utils.book_append_sheet(
                wb,
                XLSX.utils.aoa_to_sheet([
                    subjective_sheet_head(i),
                    ...this.body[i + sheet_names.length]
                        .quesions()
                        .filter(q => q.is_exist)
                        .map(que => [que.title, que.item])
                ]),
                name
            )
        )
    ...
    getAUpLoad() {
        ...
        this.body.forEach((sec, index) => {
            if (sec.config.type !== "sub") {
                let temp = {
                    name: `tizu${index + 1}`,
                    credit_per_question: sec.config.point,
                    draw_quantity: sec.config.num,
                    is_subjective: 0,
                    questions: []
                }
                temp.questions = sec.quesions().reduce(
                    (acc, val) => [
                        ...acc,
                        {
                            topic: val.title,
                            ...val.getOptTextObj(),
                            answer: val.getKeyStr()
                        }
                    ],
                    []
                )
...
export class Exam {
        ...
        this.body.concat(this.subjective).forEach(subj => {
            let block = {
                ...
                answers: subj.question.reduce((acc, val) => {
                    acc[val.id] = Array.isArray(val.stu_answer)
                        ? val.stu_answer.join("")
                        : val.stu_answer
                    return acc
                }, {})
    ...
    getOptTextArr(que, head) {
        return head.reduce((acc, val) => [...acc, que[`obj${val}`]], [])
    }
    ...
    parseToExcel() {
        ...
        try {
            ...
            this.subjective.forEach((ele, index) => {
                XLSX.utils.book_append_sheet(
                    wb,
                    XLSX.utils.aoa_to_sheet([
                        subjective_sheet_head(index),
                        ...ele.question.map(que => [que.topic, que.stu_answer])
                    ]),
                    `主观${++index}`
                )
            })
