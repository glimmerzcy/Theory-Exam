        ...
        await window.showModal({
            view: (
                <>
                    ...
                    <p>
                        已经将本次作答记录储存在本地，将会在下次您打开首页的时候自动为您提交
                    </p>
                </>
            )
        })
...
class Timer extends React.Component {
    ...
    async componentDidMount() {
        ...
        let interval = foo(duration * 60000, 1000)
...
class QuizNavigator extends React.Component {
    constructor(props) {
        ...
        getQuizUndo = () => {
            let undo = this.state.array.reduce(
                (acc, vec) => acc + (vec < 1 ? 1 : 0),
                0
            )
            if (!undo) return <></>
            else return <p style={{ color: "red" }}>还有{undo}道题没有作答！</p>
        }
    }
    componentWillMount = () => {
    ...
    move = e => {
        if (!isDrag) return
        let step = e.pageY - currentY,
            { top } = this.state,
            { clientHeight } = this.content.current
        top += step
        if (top < 100) top = 100
    ...
    render() {
        ...
        const toggle_style = {
            transform: `translateX(${
                isShow ? 0 : this.content.current.clientWidth
            }px)`
        }
            ...
            <div {...content_config}>
                ...
                <div style={toggle_style} ref={this.content}>
                    ...
                    <div style={{ marginTop: "10px" }}>
                        <div>
                            题数:{" "}
                            {array.reduce((acc, val) => (acc += val ? 1 : 0))}/
                            {this.props.num}
                        </div>
                        <div>
                            剩余时间: <Timer />
                        </div>
                        <div>考试时限: {Store.infoList.exam_paper.head.duration}min</div>
                    </div>
                </div>
            </div>
        )
    ...
    render() {
        ...
        if (type == 0) {
            ...
            func = char => () => {
                ...
                quiz.time_stamp = new Date().getTime()
                if (value) answer_set.delete(char)
                else answer_set.add(char)
        ...
        return (
            <div className="exam-content-quiz">
                <p id={index + 2}>
                    {" "}
                    {index + 1}.
                    {topic && topic.split("").map(t => <span>{t}</span>)}
                </p>
                <form name={quiz.id}>
    ...
    render() {
        ...
        return (
            <div className="exam-content-sub">
                <p class="html-text" onCopy={this.copy} id={index + 2}>
                    {index + 1}.{topic}
                </p>
                <textarea
...
class Exam extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            face_on: true,
            question: body
                .reduce((acc, val) => [...acc, ...val.question], [])
                .shuffle(),
    ...
    abandon = () => {
        ...
        window.showModal({
            ...
            onConfirm: async () => {
                ...
                window.showToast("已放弃考试！欢迎再战！", 1500, () =>
                    window.navigateTo(Route.Student.route)
                )
                await submitAnswer()
            }
    ...
    render() {
        ...
        let submit_config = {
            className: `iw-bn iw-${
                this.state.isCAPTCHA ? "confirm" : "cancel"
            }`,
            onClick: this.submit,
            style: {
                transition: "all .6s"
        ...
        return (
            <div className="exam-container">
                {this.state.prompt_on && (
                    <Prompt message="将不会保留您的作答情况" />
                )}
                ...
                <div className="exam-content">
                    <div>
                        ...
                        <div className="row aside">
                            ...
                            <span>
                                考试开始时间:{new Date().toLocaleString()}
                                ;答题时限:
                                {head.duration}min
                            </span>
                        </div>
                ...
                <div className="exam-submit">
                    <div className="pointer">
                        <MathCAPTCHA
                            onCorrect={() => this.setState({ isCAPTCHA: true })}
                            seed={233}
                        />
                    </div>
                    ...
                    <span
                        className="iw-confirm iw-bn"
                        onClick={Store.request("excel/loadoutAnswer")}
                    >
                        导出答案
                    </span>
                </div>
